...
const moveUnusedFiles = (unusedFiles) => {
  ...
  for (const file of unusedFiles) {
    ...
    switch (platform) {
      case "win32":
        undoCommand = `move "_unused\\${path.basename(file)}" "${file}"\r\n`;
        break;
      case "darwin":
      case "linux":
        undoCommand = `mv "_unused/${path.basename(file)}" "${file}"\n`;
        break;
      default:
        console.error(`Unknown platform: ${platform}`);
        process.exit(1);
...
for (const svelteFile of files) {
  ...
  let found;
  try {
    // DEPRECATED: This method does not work if run on Windows doesn’t natively have UNIX grep command
